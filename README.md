...
2.Agent_OnLoadでは、VMの各種CallBackを指定する。指定される関数は、クラスAgentCallbackHandlerに定義されている。
3.以下の指定で、Singalによるスレッドダンプを行う。
...
4.Excatの監視ツールに呼ばれるJavaクラスは、HelloWorld_forJava5のプロジェクトで定義する。
